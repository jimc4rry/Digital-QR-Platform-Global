...
# Ordered so every page listing them shows the same order. Usernames match
...
DEMO_RESTAURANT_USERNAMES = [
    ('demo_cafe', _('Cafe'), 'cup-hot'),
    ('demo_taverna', _('Taverna'), 'egg-fried'),
    ('demo_beachbar', _('Beach Bar'), 'sun'),
]


def get_demo_examples():
    """Live demo restaurants for the homepage teaser and the /examples/ showcase page -
    shared here so both pull from the same query instead of duplicating it."""
    from restaurants.models import Restaurant

    demos_by_username = {
        r.user.username: r
        for r in Restaurant.objects.filter(
            user__username__in=[u for u, _label, _icon in DEMO_RESTAURANT_USERNAMES], is_active=True,
        )
    }
    return [
        {'restaurant': demos_by_username[username], 'label': label, 'icon': icon}
        for username, label, icon in DEMO_RESTAURANT_USERNAMES
...
def qr_menu_examples(request):
    ...
    return render(request, 'tools/qr_menu_examples.html', {'examples': get_demo_examples()})
...
def _encode_url(post):
    ...
    if not value:
        raise QRInputError(_('Please enter a URL.'))
    if '://' not in value:
        value = f'https://{value}'
    try:
        URLValidator()(value)
    except ValidationError:
        raise QRInputError(_("That doesn't look like a valid URL."))
    return value
...
def _encode_text(post):
    ...
    if not value:
        raise QRInputError(_('Please enter some text.'))
    return value
...
def _encode_phone(post):
    raw = post.get('phone', '')
    if not raw.strip():
        raise QRInputError(_('Please enter a phone number.'))
    digits = _clean_phone(raw, _("That doesn't look like a valid phone number."))
    return f'tel:{digits}'
...
def _encode_email(post):
    ...
    if not address:
        raise QRInputError(_('Please enter an email address.'))
    try:
        EmailValidator()(address)
    except ValidationError:
        raise QRInputError(_("That doesn't look like a valid email address."))
    params = {}
...
def _encode_wifi(post):
    ...
    if not ssid:
        raise QRInputError(_('Please enter a network name (SSID).'))
...
def _encode_contact(post):
    ...
    if not name:
        raise QRInputError(_('Please enter a name.'))
    ...
    if phone:
        lines.append(f'TEL:{_clean_phone(phone, _("That does not look like a valid phone number."))}')
    if email:
...
def free_qr_code_generator(request):
    ...
    in-memory and returned as a data URI. Supports several QR payload types (a "type" is
    really just a different plain-text format convention - see the _encode_* functions -
